...
function omfg(msg) {
  fs.appendFileSync("/tmp/log.txt", msg);
}
    ...
    for (var sid in coverage[file].s) {
      ...
      if (coverage[file].s.hasOwnProperty(sid)) {
        ...
        if (typeof(coverage[file].statementMap[sid]) !== 'undefined') {
          ...
          omfg("      Assigned to line "+line+"\n")
        } else {
          omfg("      OH NOES!  Sid "+sid+" has no corresponding statement map entry!\n")
          // else { somehow log an error here.  Recommendations welcome. }
        }
      }
    }
    jscov[filename]["source"] = []
  }
  omfg("jsFIC returning\n");
  return jscov;
}
...
function stringifyCoverage(cov) {
  ...
  omfg(JSON.stringify(cov, null, 2))
