...
process.nextTick(function(initFunction) {
  ...
  function(err) {
    ...
    var exportedFunctionsLen = exportedFunctionsNames.length;
    var testFunction, testFunctionName;

    var failed = {};
    var succeeded = {};

    for (var i = 0; i < exportedFunctionsLen; i++) {
      testFunctionName = exportedFunctionsNames[i];

      if (testFunctionName.indexOf('test') !== 0) {
        continue;
      }
