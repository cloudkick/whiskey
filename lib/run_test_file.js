...
process.nextTick(function() {
  ...
  cwd = (process.argv.length >= 4) ? process.argv[3] : null;
...
function stringifyCoverage(cov) {
  var i, files, file, fileObj, fileLines, coverageObj = {};
...
process.on('exit', function() {
  ...
  if (typeof _$jscoverage === 'object') {
    console.log(path.join(cwd, 'jscov.js'));
    fs.writeFileSync(path.join(cwd, 'jscov.js'), stringifyCoverage(_$jscoverage));
  }
