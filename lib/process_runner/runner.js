...
var VALID_WAIT_FOR_OPTIONS = {
  ...
  'stdout': {
    'required_options': new simplesets.Set(['string']),
  },

  'socket': {
    'required_options': new simplesets.Set(['host', 'port'])
  }
};
  ...
  for (key in config) {
    if (config.hasOwnProperty(key)) {
      ...
      if (value.wait_for) {
        ...
        waitForOptions = new simplesets.Set(Object.keys(value.wait_for_options || {}));
...
ProcessRunner.prototype.findDependencies = function(testPaths, callback) {
  var self = this, exportedDependencies = new simplesets.Set([]), dependencies;
