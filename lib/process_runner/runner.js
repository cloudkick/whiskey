...
ProcessRunner.prototype.findDependencies = function(testPaths, callback) {
  var self = this, exportedDependencies = new Set([]), dependencies = new Set([]);
  ...
  function(err) {
    exportedDependencies = exportedDependencies.array();
    exportedDependencies.forEach(function(name) {
      var item = self._config[name];

      item.depends.forEach(function(name) {
        var item = self._config[name];
    ...
    callback(err, dependencies.array());
  });
};
