...
TestRunner.prototype.runTests = function(testInitFile, chdir,
  ...
  async.waterfall([
    ...
    function findDependenciesToRun(callback) {
      if (!self._onlyEssential || !self._dependencies) {
        callback(null, null);
        return;
      }

      // TODO: Pass in full resolved paths
      self._processRunner.findDependencies(self._tests, callback);
    },
