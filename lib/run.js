...
function TestRunner(options) {
  var ignored = function(ignored_test, ignored_assert) {};
  ...
  this._global_setup_procedure = ignored;
  this._global_teardown_procedure = ignored;

  var global_setup_teardown_file = options['global-setup-teardown'] || null;
  if (global_setup_teardown_file) {
    if (global_setup_teardown_file.charAt(0) !== '/') {
      global_setup_teardown_file = path.join(process.cwd(), global_setup_teardown_file);
    }

    gst_exports = require(global_setup_teardown_file);
    if (gst_exports.hasOwnProperty(constants.GLOBAL_SETUP_FUNCTION_NAME)) {
      if (underscore.isFunction(gst_exports.globalSetUp)) {
        this._global_setup_procedure = gst_exports.globalSetUp;
      }
    }
    if (gst_exports.hasOwnProperty(constants.GLOBAL_TEARDOWN_FUNCTION_NAME)) {
      if (underscore.isFunction(gst_exports.globalTearDown)) {
        this._global_teardown_procedure = gst_exports.globalTearDown;
      }
    }
  }
...
TestRunner.prototype.runTests = function(testInitFile, chdir,
  ...
  function _run_global_setup() {
    var test = new common.Test(constants.GLOBAL_SETUP_FUNCTION_NAME, self._global_setup_procedure, false);

    test.run(function(results) {
      if (results.status !== 'success') {
        self._handleTestsCompleted();
        // I don't know of a more elegant way to "fail early" when something goes wrong.
        // Throwing the exception has the desired effect, but the visual output is ugly.
        // Substance over flash strikes again! -- sam.falvo@rackspace.com
        throw new Error('Exception while invoking globalSetUp procedure:\n'+JSON.stringify(results, null, 2));
      }
    });
  }

  function _run_global_teardown() {
    var test = new common.Test(constants.GLOBAL_TEARDOWN_FUNCTION_NAME, self._global_teardown_procedure, false);
...
function run(cwd, argv) {
  ...
  if (options['coverage-files']) {
    ...
    var debug_port = parseInt(options['debug'], 10);
    if (debug_port) {
      options['debug'] = debug_port;
    } else {
      options['debug'] = 5858;
    }
