...
var getTestReporter = require('./reporters/index').getTestReporter;
var getCoverageReporter = require('./reporters/index').getCoverageReporter;

var exitCode = 0;

function TestRunner(options) {
  var testReporter, coverageReporter,
      testReporterOptions, coverageReporterOptions;
  ...
  this._testReporter = getTestReporter(testReporter, this._tests,
                                       testReporterOptions);

  if (options['coverage-reporter']) {
      coverageReporter = options['coverage-reporter'] || constants.DEFAULT_COVERAGE_REPORTER;
      coverageOptions = {
        'directory': options['coverage-dir'],
      };

      this._coverageReporter = getCoverageReporter(coverageReporter, this._tests,
                                                   coverageOptions);
  }
  else {
...
function run(cwd, argv) {
  ...
  if (options.tests && options.tests.length > 0) {
    ...
    if (scopeLeaks && (concurrency > 1)) {
        throw new Error('--scope-leaks can only be used in sequential mode ' +
                        '(--concurrency 1)');
    }
