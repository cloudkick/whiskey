...
function TestRunner(tests, socketPath, reporter, coverageReporter, coverage, verbosity) {
  this._tests = tests;
...
TestRunner.prototype._spawnTestProcess = function(filePath,
  ...
  var args = [runFilePath, filePath, this._socketPath, cwd, libCovDir, chdir,
              customAssertModule, testInitFile, timeout, concurrency];
...
function run(cwd, argv) {
  ...
  if (options.tests && options.tests.length > 0) {
    ...
    socketPath = options['socket-path'] || constants.DEFAULT_SOCKET_PATH;
    concurrency = options['sequential'] ? 1 : options['concurrency'];
    runner = new TestRunner(options['tests'], socketPath, testReporter, coverageReporter,
                            options['coverage'],
                            options['verbosity']);
