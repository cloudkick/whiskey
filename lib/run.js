...
function TestRunner(tests, reporter, chdir, failFast, concurrency, timeout,
                    verbosity) {
  this._tests = tests;
  this._reporter = reporter;
  this._chdir = chdir;
  this._failFast = failFast;
  this._concurrency = concurrency || constants.DEFAULT_CONCURRENCY;
  this._timeout = timeout || constants.DEFAULT_TEST_TIMEOUT;
  this._verbosity = verbosity || constants.DEFAULT_VERBOSITY;

  this._server = null;
  this._testFileCallbacks = {};
...
TestRunner.prototype.runTests = function() {
  ...
  function onBound() {
    ...
    async.forEachSeries(self._tests, function(testFilePath, callback) {
      self._testFileCallbacks[testFilePath] = callback;
      self._spawnTestProcess(testFilePath);
    },
...
TestRunner.prototype._spawnTestProcess = function(filePath) {
  ...
  var args = ['run_test_file.js', filePath, cwd, this._timeout,
              this._concurrency];
...
TestRunner.prototype._handleConnection = function(runner, connection) {
  var data = '';
  var dataString, endMarkIndex, testFileCallback;
  ...
  function onEnd(data) {
    ...
    testFileCallback = runner._testFileCallbacks[filePath];
    if (testFileCallback) {
      testFileCallback();
      delete runner._testFileCallbacks[filePath];
    }
  }
...
function run(cwd, argv) {
  ...
  if (options.tests && options.tests.length > 0) {
    ...
    var reporter = reporters.getReporter('cli', options.tests, reporterOptions);
    var runner = new TestRunner(options.tests, reporter, options.chdir,
                                options.failFast, options.concurrency,
