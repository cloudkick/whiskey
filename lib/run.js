 ...
 */
var executeTest = function(cwd, chdir, testInitFile, filePath, failfast, verbosity, callback) {
  var fileName = path.basename(filePath);

  testUtil.printMsg(sprintf('Running test file: %s', fileName), verbosity, 3);

  var args = ['run_test_file.js', filePath, cwd, chdir, testInitFile];
...
var runTests = function(cwd, initFile, testInitFile, chdir, tests, failFast,
  ...
  // block there.
  if (initFile) {
    common.runInitFunction(initFile);
  }

  async.forEachSeries(tests, function(test, callback) {
    executeTest(cwd, chdir, testInitFile, test, failFast, verbosity, callback);
  },
...
var run = function(cwd, argv) {
  ...
  if (tests.length > 0) {
    runTests(cwd, initFile, testInitFile, chdir, tests, failFast, verbosity);
  }
  else {
    sys.puts(p.banner);
  }
