...
function TestRunner(tests, reporter, chdir, verbosity) {
  this._tests = tests;
...
TestRunner.prototype.runTests = function(testInitFile, timeout, concurrency,
                                         failFast) {
  var self = this;
  ...
  function onBound() {
    ...
    async.forEachSeries(self._tests, function(filePath, callback) {
      var child = self._spawnTestProcess(filePath, testInitFile, timeout,
                                         concurrency);
...
TestRunner.prototype._spawnTestProcess = function(filePath, testInitFile,
  ...
  var args = ['run_test_file.js', filePath, cwd, testInitFile, timeout,
              concurrency];
...
function run(cwd, argv) {
  ...
  if (options.tests && options.tests.length > 0) {
    ...
    runner.runTests(options['test-init-file'], options['timeout'],
                    options['concurrency'], options['fail-fast']);
  }
  else {
    sys.puts(p.banner);
  }
