...
var executeTest = function(cwd, chdir, testInitFile, filePath, failfast, timeout,
  ...
  if(verbosity > 2) {
    sys.puts('Running test file: ' + args.join(' '));
  }
  ...
  child.on('exit', function(code) {
    ...
    if (!timedOut) {
      ...
      if (cov) {
        coverage_results[fileName] = JSON.parse(fs.readFileSync(path.join(cwd, 'jscov.js')));
        fs.unlinkSync(path.join(cwd, 'jscov.js'));
      }
    }
    else {
      timeouts++;
  ...
  var executeTests = function() {
    ...
    function(err) {
      ...
      if (cov) {
        ...
        coverage.htmlCoverage(path.join(cwd, 'cov-html'), results);
      }
    });
  };
