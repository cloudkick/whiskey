...
function TestRunner(options) {
  ...
  this._independentTests = options['independent-tests'];
  this._maxSuites = options['max-suites'];
  ...
  this._globalSetupProcedure = null;
  this._globalTeardownProcedure = null;

  var globalSetupTeardownFile = options['global-setup-teardown'] || null;
  if (globalSetupTeardownFile) {
    if (globalSetupTeardownFile.charAt(0) !== '/') {
      globalSetupTeardownFile = path.join(process.cwd(), globalSetupTeardownFile);
    }

    gstExports = require(globalSetupTeardownFile);
    if (gstExports.hasOwnProperty(constants.GLOBAL_SETUP_FUNCTION_NAME)) {
      if (underscore.isFunction(gstExports.globalSetUp)) {
        this._globalSetupProcedure = gstExports.globalSetUp;
      }
    }
    if (gstExports.hasOwnProperty(constants.GLOBAL_TEARDOWN_FUNCTION_NAME)) {
      if (underscore.isFunction(gstExports.globalTearDown)) {
        this._globalTeardownProcedure = gstExports.globalTearDown;
      }
    }
  }
...
TestRunner.prototype.runTests = function(testInitFile, chdir,
  ...
  function runAllSuites(callback) {
    self._testReporter.handleTestsStart();

    async.series([
      async.forEachLimit.bind(null, self._independentTests, self._maxSuites, runSuite),
  ...
  function onBound() {
    async.series([
      function runGlobalSetup(callback) {
        var test;

        if (!self._globalSetupProcedure) {
          callback();
          return;
        }

        test = new common.Test(constants.GLOBAL_SETUP_FUNCTION_NAME, self._globalSetupProcedure, false);
        test.run(function(results) {
          if (results.status !== 'success') {
            throw new Error('Exception while invoking globalSetUp procedure: \n'+JSON.stringify(results, null, 2));
          }
      ...
      function runGlobalTeardown(callback) {
        var test;

        if (!self._globalTeardownProcedure) {
          callback();
          return;
        }

        test = new common.Test(constants.GLOBAL_TEARDOWN_FUNCTION_NAME, self._globalTeardownProcedure, false);
        test.run(function(results) {
          if (results.status !== 'success') {
            console.error('WARNING: Failures detected in the globalTearDown procedure.\n'+JSON.stringify(results, null, 2));
          }
...
TestRunner.prototype._spawnTestProcess = function(filePath,
  ...
  if (this._debug) {
    args.push('--debug-brk=' + this._debug);
    console.log('Debugger listening on port ' + this._debug);
  }
...
function run(cwd, argv) {
  ...
  if (options['coverage-files']) {
    ...
    if (intersection.length > 0) {
      console.log(sprintf('The following tests cannot appear in both --tests and --independen-tests: %s', intersection));
      process.exit(1);
    }
    ...
    if (options['debug'] && options.tests.length > 1) {
      ...
      generateMakefile(options.tests, options['makefile-path'], function(err) {
        ...
        console.log(sprintf('Makefile has been saved in %s.', options['makefile-path']));
      });
