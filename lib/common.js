...
Test.prototype.run = function(callback) {
  ...
  testObj = this._getTestObject(finishFunc);

  try {
    this._testFunction(testObj, this._getAssertObject());
  }
  catch (err) {
...
Test.prototype._getAssertObject = function() {
  return assert.getAssertModule(this._testName);
};
...
TestFile.prototype.runTests = function(callback) {
  ...
  function handleEnd() {
    ...
    var resultObj = self.getResultObject(null);
    var testsLen = self._tests.length;
    var exportedFunctionsLength = exportedFunctionsNames.length;

    if (setUpFunc) {
      exportedFunctionsLength++;
    }
    ...
    if (testsLen !== exportedFunctionsLength) {
      // Timeout
  ...
  var timeoutId = setTimeout(handleEnd, this._timeout);
...
TestFile.prototype.addUncaughtException = function(err) {
  ...
  if (err.testName) {
    test = new Test(err.testName, null);
    test._markAsFailed(err);
    this.addTest(test);
    return;
  }

  // User did not use our assert module, so it's hard to figure out where the
  // exception origination from. Just add it to the _uncaughtExceptions array and
  // reporter it as "uncaught exception".
...
function reportTestResults(socketPath, resultObject) {
  ...
  connection.on('connect', function onConnect() {
    var resultString = sprintf('%s%s', JSON.stringify(resultObject),
